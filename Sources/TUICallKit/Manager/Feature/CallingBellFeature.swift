...
class CallingBellFeature: NSObject, AVAudioPlayerDelegate {
    ...
    func startPlayMusic(type: CallingBellType) {
#if SWIFT_PACKAGE
        guard let bundlePath = Bundle.module.resourceURL?.path else { return }
#else
        guard let bundlePath = CallKitBundle.getTUICallKitBundle().bundlePath + "/AudioFile" else { return }
#endif
        switch type {
        case .CallingBellTypeHangup:
            let path = bundlePath + "/phone_hangup.mp3"
        ...
        case .CallingBellTypeCalled:
            ...
            var path = bundlePath + "/phone_ringing.mp3"
        ...
        case .CallingBellTypeDial:
            let path = bundlePath + "/phone_dialing.m4a"
